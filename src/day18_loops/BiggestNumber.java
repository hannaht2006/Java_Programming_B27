...
public class BiggestNumber {
    public static void main(String[] args) {
        ...
        int max =-2147483648;
        int min = 2147483647;
        int num =0; // to keep track of the iteration
        while (num++<5) { // this loop will run for 5 times, start  from 1  and go until num <= 5
            System.out.println("Enter a number");
            int inputNum = input.nextInt();
            if (inputNum > max) {
                max = inputNum;//if the new number from the console is bigger than our current biggest number, we found a new max and should reassign it to the variable
            }
            if (inputNum < min) {
                min = inputNum;
